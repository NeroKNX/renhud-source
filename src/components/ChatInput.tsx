...
export function ChatInput({ onSend, disabled, isDeep, onToggleDeep }: ChatInputProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (attachedFiles.length > 0) {
      fileAttachments = await convertFiles(attachedFiles);
    }
      ...
      {attachedFiles.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-2">
          {attachedFiles.map((file, i) => (
            <div key={i} className="flex items-center gap-1.5 px-2 py-1.5 bg-[#1a1d2e] border border-[#2d3250] rounded-lg text-xs font-mono text-gray-300">
              ...
              <button type="button" onClick={() => removeFile(i)} className="p-0.5 hover:bg-[#2d3250] rounded">
      ...
      <form onSubmit={handleSubmit}>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={disabled}
            className="p-1.5 sm:p-2 rounded-lg border border-transparent hover:border-[#2d3250] text-gray-400 hover:text-gray-300 transition-colors disabled:opacity-50"
