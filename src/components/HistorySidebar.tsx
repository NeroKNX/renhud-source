...
export function HistorySidebar({ isOpen, onClose, sessions, currentSessionId, onSelectSession, onDeleteSession, onRenameSession, onToggleFavorite }: HistorySidebarProps) {
  ...
  const handleSelect = (id: string) => { onSelectSession(id); navigate(`/chat/${id}`); onClose(); };
  const handleDelete = (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    if (confirm('¿Eliminar esta conversación?')) { onDeleteSession(id); if (currentSessionId === id) navigate('/chat'); }
  };
  const handleStartEdit = (e: React.MouseEvent, session: ChatSession) => { e.stopPropagation(); setEditingId(session.id); setEditTitle(session.title); };
  const handleSaveEdit = (id: string) => { if (editTitle.trim()) onRenameSession(id, editTitle.trim()); setEditingId(null); };
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50">
          ...
          <motion.div
            ...
            className="absolute left-0 top-0 bottom-0 w-80 md:w-96 flex flex-col border-r shadow-[0_0_50px_rgba(0,0,0,0.5)]"
          ...
          >
            <div className="px-5 py-6 border-b" style={{ borderColor: 'var(--ren-border)' }}>
              ...
              <div className="relative">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar conversaciones..."
                  className="w-full border rounded-lg pl-10 pr-4 py-2 text-sm font-mono text-gray-100 placeholder-gray-600 focus:border-[#4f46e5] outline-none"
                  style={{ backgroundColor: 'var(--ren-bg-primary)', borderColor: 'var(--ren-border-light)' }} />
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto ren-scrollbar">
              {filtered.length === 0 ? (
                <div className="text-center py-12 px-4">
                  <p className="text-sm text-gray-500 font-mono">{search ? 'No se encontraron conversaciones' : 'No hay conversaciones aún'}</p>
                </div>
              ) : filtered.map(session => (
                <motion.div key={session.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
                  className={`px-5 py-3 border-b cursor-pointer transition-colors ${currentSessionId === session.id ? 'bg-[var(--ren-bg-hover-strong)]' : 'hover:bg-[var(--ren-bg-hover)]'}`}
                  style={{ borderColor: 'var(--ren-border-subtle)' }}
                  onClick={() => handleSelect(session.id)}>
                  {editingId === session.id ? (
                    <div onClick={e => e.stopPropagation()}>
                      <input value={editTitle} onChange={e => setEditTitle(e.target.value)}
                        onKeyDown={e => { if (e.key === 'Enter') handleSaveEdit(session.id); if (e.key === 'Escape') setEditingId(null); }}
                        onBlur={() => handleSaveEdit(session.id)} autoFocus
                        className="w-full border border-[#4f46e5] rounded px-2 py-1 text-sm text-gray-100 font-mono outline-none"
                    ...
                    <>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1.5 min-w-0 flex-1">
                          ...
                          <p className="text-sm font-mono text-gray-200 truncate">{session.title}</p>
                        </div>
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity ml-2">
                          <button onClick={e => { e.stopPropagation(); onToggleFavorite(session.id); }}
                          ...
                          <button onClick={e => handleStartEdit(e, session)} className="p-1 hover:bg-[#2d3250] rounded text-gray-500 hover:text-gray-300" title="Renombrar">
                          ...
                          <button onClick={e => handleDelete(e, session.id)} className="p-1 hover:bg-[#2d3250] rounded text-gray-500 hover:text-red-400" title="Eliminar">
