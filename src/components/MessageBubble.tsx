...
const MODEL_LABELS: Record<string, { label: string; color: string }> = {
  'claude-sonnet': { label: 'Claude', color: 'text-[#818cf8]' },
  deepseek: { label: 'DeepSeek', color: 'text-emerald-400' },
  llama: { label: 'Llama', color: 'text-orange-400' },
  'llama-70b': { label: 'Llama', color: 'text-orange-400' },
  'mistral-7b': { label: 'Mistral', color: 'text-orange-400' },
};
...
export function MessageBubble({ message, onEdit, onRegenerate }: MessageBubbleProps) {
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => { if (bubbleRef.current && !bubbleRef.current.contains(e.target as Node)) setShowActions(false); };
  ...
  const handleCopy = async () => {
    try { await navigator.clipboard.writeText(message.text); setCopied(true); setTimeout(() => setCopied(false), 2000); }
      ...
      const ta = document.createElement('textarea'); ta.value = message.text; ta.style.position = 'fixed'; ta.style.left = '-9999px';
      document.body.appendChild(ta); ta.select(); document.execCommand('copy'); document.body.removeChild(ta);
  ...
  const formatTime = (ts: string) => {
    ...
    if (diffMin < 2) return 'Ahora'; if (diffMin < 60) return `Hace ${diffMin} min`;
    const diffHr = Math.floor(diffMin / 60); if (diffHr < 24) return `Hace ${diffHr}h`;
  ...
  const modelInfo = !message.isUser && message.model ? MODEL_LABELS[message.model] : null;

  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, ease: [0.25, 0.1, 0.25, 1] }}
      ...
      <div className={`relative flex flex-col max-w-[88%] sm:max-w-[82%] md:max-w-[68%] min-w-0 ${message.isUser ? 'items-end' : 'items-start'}`}>
        <div ref={bubbleRef} onClick={() => setShowActions(v => !v)}
            ...
            <div className="flex flex-wrap gap-2 mb-2.5">
              {message.files.map((file, i) =>
                file.type.startsWith('image/') ? (
                  ...
                  <video key={i} controls className="max-w-full rounded-xl border border-[#2d3250] max-h-72" src={file.data}><track kind="captions" /></video>
                ) : (
          ...
          <div className="flex items-end justify-between mt-1 sm:mt-1.5">
            <div className="flex items-center gap-1.5 leading-none">
              ...
              {modelInfo && <span className={`text-[8px] sm:text-[9px] font-mono ${modelInfo.color}`}>{modelInfo.label}</span>}
            </div>
            <div className="flex items-center gap-1.5 leading-none">
              {!message.isUser && message.isDeep && (
                <span className="inline-flex items-center gap-0.5 px-1 py-0.5 bg-[#4f46e5]/12 border border-[#4f46e5]/20 rounded-full">
                  <svg width="7" height="7" viewBox="0 0 24 24" fill="none" stroke="#4f46e5" strokeWidth="3"><path d="M12 18v-5.25M12 10.5h.01M21.5 12a9.5 9.5 0 1 1-19 0 9.5 9.5 0 0 1 19 0Z"/></svg>
        ...
        <div className={`flex gap-1 mt-0.5 transition-all duration-200 ${showActions ? 'opacity-100' : 'opacity-0 sm:group-hover:opacity-100'}`}>
          <button onClick={(e) => { e.stopPropagation(); handleCopy(); }} className="p-0.5 rounded text-gray-500/60 hover:text-[#818cf8] transition-colors active:scale-90" title="Copiar">
            {copied ? <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#22c55e" strokeWidth="2"><path d="M20 6L9 17l-5-5"/></svg>
          ...
          {message.isUser && onEdit && (
            <button onClick={(e) => { e.stopPropagation(); const t = prompt('Editar mensaje:', message.text); if (t && t !== message.text) onEdit(t); }}
              className="p-0.5 rounded text-gray-500/60 hover:text-[#818cf8] transition-colors active:scale-90" title="Editar">
          ...
          {!message.isUser && onRegenerate && (
            <button onClick={(e) => { e.stopPropagation(); onRegenerate(); }} className="p-0.5 rounded text-gray-500/60 hover:text-[#818cf8] transition-colors active:scale-90" title="Regenerar">
      ...
      <style>{`
        ...
        .ren-content h1 { font-size: 1.05em; } .ren-content h2 { font-size: 1em; }
        .ren-content h3 { font-size: 0.95em; color: #a5b4fc; } .ren-content h4 { font-size: 0.9em; color: #c7d2fe; }
