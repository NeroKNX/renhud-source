    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4" style={{ backgroundColor: 'var(--ren-bg-primary)' }}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full">
        <div className="mb-8 text-center">
          <div className="w-20 h-20 rounded-full flex items-center justify-center overflow-hidden mx-auto mb-6 border-2 border-[#4f46e5]/40"
            style={{ backgroundColor: 'var(--ren-bg-tertiary)', boxShadow: '0 0 30px rgba(79,70,229,0.4)' }}>
        ...
        <form onSubmit={handleSubmit} className="w-full max-w-md mx-auto border rounded-2xl p-8 space-y-5 shadow-[0_0_50px_rgba(0,0,0,0.3)]"
          style={{ backgroundColor: 'var(--ren-bg-secondary)', borderColor: 'var(--ren-border)' }}>
          {error && <div className="bg-red-500/10 border border-red-500/30 rounded-lg px-4 py-2 text-sm text-red-400 font-mono">{error}</div>}

          <div className="space-y-2">
            <label className="text-xs font-mono text-gray-400 uppercase tracking-wider">Nombre</label>
            <input type="text" value={username} onChange={e => setUsername(e.target.value)} placeholder="Tu nombre" required
              className="w-full border rounded-lg px-3 py-2.5 text-sm font-mono text-gray-100 placeholder-gray-600 focus:border-[#4f46e5] outline-none"
              style={{ backgroundColor: 'var(--ren-bg-input-dark)', borderColor: 'var(--ren-border-light)' }} />
          </div>
          <div className="space-y-2">
          ...
          <p className="text-xs font-mono text-gray-500 text-center">
            ¿Ya tienes cuenta? <button type="button" onClick={() => navigate('/login')} className="text-[#818cf8] hover:underline">Iniciar sesión</button>
          </p>
        </form>

        <a href="/notes.html" className="block mt-4 text-center text-xs font-mono text-gray-500 hover:text-gray-300">📝 Notas de REN</a>

        <div className="mt-3 text-center">
          <button onClick={() => {
            setUser({ user_id: 'guest_' + Math.random().toString(36).slice(2, 10), isGuest: true, name: 'Invitado' });
            navigate('/chat');
